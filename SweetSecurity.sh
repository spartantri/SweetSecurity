...
read -p "Install local ELK stack? yes/no: " ELK 
ELK=${ELK:-yes}
...
sudo apt-get install -y --force-yes python-ipaddress
...
#Install ossec
cd $home_path
path_ossec="${home_path}/ossec-hids-3.2.0"
#sudo wget https://bintray.com/artifact/download/ossec/ossec-hids/ossec-hids-2.8.3.tar.gz
sudo wget https://github.com/ossec/ossec-hids/archive/3.2.0.tar.gz -O ossec-hids-3.2.0.tar.gz
sudo tar -zxvf ossec-hids-3.2.0.tar.gz
sudo ln -s /opt/ossec/etc /etc/ossec
sudo ln -s /opt/ossec/logs /var/log/ossec
sudo cp $my_path/preloaded-vars.conf $path_ossec/etc
sudo sed -i -- "s/EMAIL_USER/$emailAddr/g" $path_ossec/etc/preloaded-vars.conf
sudo sed -i -- "s/SMTP_HOST/$smtpHost/g" $path_ossec/etc/preloaded-vars.conf
sudo sed -i -- "s/\(<email_to>\).*\(<\/email_to>\)/\1${emailAddr}\2/g" $path_ossec/etc/ossec.conf
sudo sed -i -- "s/\(<smtp_server>\).*\(<\/smtp_server>\)/\1${smtpHost}\2/g" $path_ossec/etc/ossec.conf
